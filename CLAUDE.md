...
- [x] Fase 8: Sandboxes — SubprocessSandbox + DockerSandbox, SandboxPolicy (5 perfis: UNTRUSTED/SKILL_DEV/DEFAULT/TRUSTED/OPEN), exec_tool (C7 compliant, single entry point), NetworkPolicy com intercept real, migration 009_sandbox_executions.sql, 81 testes passando
- [x] Fase 9: Skills auto-geradas estilo Voyager — SkillManifestF9 (Pydantic, slug snake_case, NetworkPolicy sem wildcard), SkillRegistry (PostgreSQL, bytea embeddings, cosine Python), SkillValidator (ast.parse + ruff via sandbox + smoke run via exec_tool, C7 compliant), SkillSynthesizer (scan cluster ≥5 execuções cosine≥0.85, LLM + fallback template), SkillPromoter (integra Crítico F7), SkillRunner (single entry via exec_tool, _RUNTIME_SCRIPT), SkillDecayManager (30d→deprecated, >40% erro→flagged, ≥20 sucesso→mature), migration 010_skills.sql, decay job APScheduler diário 3h, 8 rotas REST /api/v1/skills/*, CLI `agent skills list/show/run/promote/reject/archive/stats/synthesize`, 72 testes F9 passando
- [x] Fase 10: Deploy VPS — Supervisor (fork+monitor, backoff exponencial, flapping detection sliding window 10 restarts/10min), 4 workers (api/orchestrator/scheduler/heartbeat), health endpoints (/live /ready /deep com noop skill via exec_tool), 12 métricas Prometheus, logging JSON rotacionado com redaction, backup (pg_dump+sqlite3.backup+tar) + restore, install/uninstall systemd, sd_notify via UNIX socket, migration 011_deploy.sql, CLI `agent deploy install/uninstall/start/stop/restart/status/logs/backup/restore/doctor/upgrade`, 106 testes passando
- [x] Fase 11: Web UI — FastAPI web module (server/auth/metrics/routes/adapters), WS multiplexado /api/v1/stream (heartbeat 20s, backpressure 100, close 4001), auth HMAC 5s cache, rate limit 60 req/s, CSP estrita, 8 painéis terminal (HTML/CSS/JS vanila), CLI agent web, migration 012_web.sql, 113+12 testes passando, cobertura 87%, scripts/run_webui.py para dev local, smoke E2E 8/8 endpoints OK
- [x] Fase 12: Canais extras — Discord (discord.py, guild+allowlist, embed), Slack (slack-bolt Socket Mode, thread_ts, files_upload_v2), Email (IMAP IDLE, SMTP, anti-loop C8, SPF/DKIM C9), ChannelRouter (allowlist+rate limit sliding window, persist in/out), ChannelAdapter ABC, IncomingMessage/OutgoingMessage, metrics agent_channel_* (6 counters/gauges), redact_secrets structlog processor (C10), gating APPROVAL_CHANNELS (C6), migration 013_channel_messages.sql, bootstrap_channels no server.py lifespan, /health canal status, 167 testes passando, cobertura 85.4%
- [x] Fase 13: Fine-tuning local periódico (LoRA) — LoraTrainer (Unsloth+fallback peft), BenchmarkRunner (6 eixos, 62 tasks, juiz Claude), CheckpointGate (3 gates: safety/per-axis/overall), CheckpointRegistry (ativação atômica, rollback), migration 014, CLI `agent finetune`, 70 testes, C1–C14 cobertos
- [x] Fase Infra: CI GitHub Actions verde, auto-migrations (agent.db.migrate + stamp_all), CLI `agent db migrate`, Dockerfile F11.1 (webui/public no container), ruff format global, pytest-cov, 1158 testes passando, tag fase-infra-done
- [ ] Fase 14+: RLAIF
