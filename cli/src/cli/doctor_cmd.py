...
async def _run_diagnostics() -> None:
    ...
    if failed == 0:
        ...
        console.print(
            f"[bold yellow]{failed} problema(s) encontrado(s).[/bold yellow] ({passed}/{len(checks)} checks passaram)"
        )
        sys.exit(1)
...
def _check_python() -> CheckResult:
    ...
    return CheckResult(
        "Python",
        False,
        f"versão {v.major}.{v.minor} — requer 3.11+",
    ...
    if provider == "ollama":
        ...
        try:
            ...
            return CheckResult(
                "Provider ativo",
                False,
                health.message or "Ollama não responde",
...
def _check_docker() -> CheckResult:
    ...
    return CheckResult(
        "Docker",
        True,
        "não instalado [dim](opcional)[/dim]",
        hint="",
    )
